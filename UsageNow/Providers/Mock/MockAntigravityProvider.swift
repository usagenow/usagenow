...
/// local activity — like the real provider while Antigravity CLI runs.
struct MockAntigravityProvider: UsageProvider {
    ...
    func snapshot() throws -> ProviderSnapshot {
        ...
        case .unavailable:
            return ProviderSnapshot(provider: .antigravity, status: .available, quotaUnavailableReason: .toolNotRunning, updatedAt: date)
