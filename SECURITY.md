...
- For Antigravity, UsageNow asks Antigravity CLI's own language server on `127.0.0.1` while `agy` runs. It finds the port from socket metadata (`lsof`), never reads the Google sign-in `agy` saved or other processes' command lines, and sends no credentials. Certificate checks are skipped for loopback addresses only.
